...
const outPath = path.join(root, "src/data/molde-cruise-schedules.generated.json");

const payload = JSON.parse(readFileSync(sourcePath, "utf8"));
const rows = (payload.rows || []).filter((r) => r.port === "molde");
...
const years = {};
for (const r of rows) {
  years[r.arrival_date.slice(0, 4)] =
    (years[r.arrival_date.slice(0, 4)] || 0) + 1;
}
