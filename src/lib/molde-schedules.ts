import schedulePayload from "@/data/molde-cruise-schedules.generated.json";

export type MoldeScheduleRow = {
  port: string;
  ship: string;
  cruise_line: string;
...
export type MoldeScheduleEntry = {
  date: string;
  ship: string;
  arrival: string;
  departure: string;
...
type SchedulePayload = {
  ...
  rows: MoldeScheduleRow[];
};

const payload = schedulePayload as SchedulePayload;

export const moldeScheduleIntegrity = payload.integrity;
export const moldeScheduleMeta = {
  port: payload.port,
...
function toEntry(row: MoldeScheduleRow): MoldeScheduleEntry {
  return {
...
export function getAllMoldeRows(): MoldeScheduleRow[] {
  return payload.rows;
}

export function getMoldeEntries(): MoldeScheduleEntry[] {
  return payload.rows.map(toEntry);
}

export function getMoldeEntriesForMonthKey(monthKey: string): MoldeScheduleEntry[] {
  return payload.rows
...
export function getMoldeMonthKeysWithCalls(): string[] {
  return [...new Set(payload.rows.map((row) => row.arrival_date.slice(0, 7)))].sort();
}

export function getMoldeMonthSummaries() {
