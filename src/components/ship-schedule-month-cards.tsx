...
import {
  ...
  type getMoldeMonthSummaries,
} from "@/lib/molde-schedules";

type MonthSummary = ReturnType<typeof getMoldeMonthSummaries>[number];
  ...
  return (
    <div className="space-y-10">
            ...
            {yearMonths.map((month) => (
              <li key={month.slug}>
                <Link
                  ...
                  className="block rounded border border-[var(--border-light)] bg-white px-4 py-4 transition hover:border-[var(--norway-blue)]"
                >
                  <p className="font-semibold text-slate-900">{month.label}</p>
                  <p className="mt-1 text-sm text-slate-600">
                    {month.callCount} published call
                    {month.callCount === 1 ? "" : "s"} · {month.shipCount} ship
                    {month.shipCount === 1 ? "" : "s"}
                  </p>
                </Link>
              </li>
      ...
      <p className="text-sm leading-6 text-slate-500">
        {scheduleDisclaimer} Coverage runs through{" "}
        {moldeScheduleIntegrity.lastDate
          ? formatScheduleDate(moldeScheduleIntegrity.lastDate)
          : "the last published call"}
        . Browse from the{" "}
        <Link href={shipScheduleHubPath} className="content-link">
          Molde ship schedule hub
        </Link>
        .
      </p>
    </div>
